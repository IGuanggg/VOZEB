<p align="center">
  <img src="web/public/logo.svg?v=1.0.0-white" width="108" alt="VOZEB logo">
</p>

<h1 align="center">VOZEB</h1>

<p align="center">
  <a href="https://github.com/IGuanggg/VOZEB"><img src="https://img.shields.io/github/stars/IGuanggg/VOZEB?style=flat-square&logo=github" alt="GitHub stars"></a>
...
> [!IMPORTANT]
> 本仓库是基于 [csyqlz/vozeb](https://github.com/csyqlz/vozeb) 的二次开发版本，并保留 GitHub Fork 关系。项目继续遵循上游 AGPL-3.0 许可证，不代表上游作者对本分支提供维护或背书。

## 二次开发说明

当前分支面向多人共享的 AI 无限画布场景进行了持续改造，主要新增或调整：

- 多用户图片任务队列、全局并发与单用户公平调度，适配 Chat2API 账号池。
- 单次多图拆分请求、坏账号换号、超时处理和异步图片任务回填。
- 3D 导演台嵌入、工程状态持久化、机位截图回传画布和手机端适配。
- 图片节点编辑器、蒙版编辑、裁剪、切图、快速放大与浏览器本地 AI 超清。
- 图片引用持久化、节点提及、图片工具栏自定义和移动端无限画布手势修复。
- 管理后台、用户系统、积分、公告、模型渠道和系统级 API 配置扩展。

3D 导演台集成参考 [jiguang132/storyai-3d-director-desk](https://github.com/jiguang132/storyai-3d-director-desk)。本地超分功能参考 [liwei9745/GenBox](https://github.com/liwei9745/GenBox) 与 [jianjianai/4k-image-api](https://github.com/jianjianai/4k-image-api)，当前实现使用浏览器本地 UpscalerJS / ESRGAN Slim，并保留对应模型许可证。完整来源与许可证说明见 [UPSTREAM.md](UPSTREAM.md)。

<p align="center">
  二开演示站：<a href="https://image.0909106.xyz">image.0909106.xyz</a>
</p>
...
VOZEB 当前版本为 `v1.0.0`，这是基于原创开源画布项目继续开发的二开版本。感谢原创作者 basketikun 对无限画布、AI 创作工作流、Canvas Agent 和 Codex 插件能力的开源贡献。

版本更新记录请查看 [GitHub Releases](https://github.com/IGuanggg/VOZEB/releases)。
...
```bash
git clone https://github.com/IGuanggg/VOZEB.git
cd VOZEB
docker compose build
docker compose up -d
```

本二开仓库默认从当前源码构建 Docker 镜像。端口绑定使用 `VOZEB_BIND_HOST=0.0.0.0`，当前 compose 对外端口为 `3020`，账号、后台设置、签到记录和公共提示词会保存在 `vozeb-data` 数据卷里。生产环境请自行配置 `.env`、HTTPS 反向代理和模型渠道，不要把真实 API Key 提交到仓库。

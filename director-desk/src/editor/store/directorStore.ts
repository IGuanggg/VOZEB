...
function buildCameraCaptures(camera: DirectorCameraShot, dataUrls: string[]) {
  ...
  const lastCaptureIndex = existingCaptures.reduce((maximum, capture) => Math.max(maximum, capture.index), 0);

  return dataUrls.map((dataUrl, indexOffset): DirectorCameraCapture => {
    const captureIndex = lastCaptureIndex + indexOffset + 1;
      ...
      commitMutation((state) => {
        ...
        const cameras = state.project.cameras.map((camera) => {
          ...
          return {
            ...
            captures: [...(camera.captures ?? []), ...nextCaptures].slice(-MAX_CAMERA_CAPTURES),
          };
        });

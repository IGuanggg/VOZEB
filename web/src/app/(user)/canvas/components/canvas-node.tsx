...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    return (
        ...
        >
            ...
            {showPanel && renderPanel ? <div className="absolute left-1/2 top-full z-[70] w-[min(500px,calc(100vw-24px))] -translate-x-1/2 pt-4">{renderPanel(data)}</div> : null}
        </div>
    );
...
function ConnectionHandleDot({ side, visible, onConnectStart }: { side: "left" | "right"; visible: boolean; onConnectStart: (event: React.MouseEvent | React.PointerEvent) => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];

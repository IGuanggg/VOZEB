...
it("appends camera captures with sequential camera-shot names", () => {
  ...
  expect(localStorage.getItem("storyai-3d-director-desk-demo")).not.toContain("data:image/png;base64");
});

it("keeps only the latest camera captures in the active session", () => {
  useDirectorStore.setState(createInitialDirectorState());

  useDirectorStore.getState().addCameraCaptures(
    "cam_1",
    Array.from({ length: 30 }, (_, index) => `data:image/png;base64,${index + 1}`)
  );

  const captures = useDirectorStore.getState().project.cameras[0]?.captures ?? [];
  expect(captures).toHaveLength(24);
  expect(captures[0]?.index).toBe(7);
  expect(captures[23]?.index).toBe(30);
});

<p align="center">
  <img src="web/public/logo.svg?v=0.6.2-white" width="108" alt="VOZEB logo">
</p>
...
<p align="center">
  ...
  <a href="VERSION"><img src="https://img.shields.io/badge/version-v0.6.2-2563eb?style=flat-square" alt="Version"></a>
...
VOZEB 当前版本为 `v0.6.2`，这是基于原开源项目 [basketikun/infinite-canvas](https://github.com/basketikun/infinite-canvas) 继续开发的二开版本。感谢原创作者 basketikun 对无限画布、AI 创作工作流、Canvas Agent 和 Codex 插件能力的开源贡献。
...
- 用户系统：支持账号密码注册登录、邮箱注册开关、SMTP 邮箱服务配置、管理员后台、用户角色、账号状态、每日额度和签到奖励。
...
如果服务器拉取 `ghcr.io/csyqlz/vozeb:latest` 时提示 `denied`，说明 GitHub Packages 里的容器镜像还没有设为 Public，或者服务器没有登录 GHCR。公开部署推荐到 GitHub 仓库右侧 `Packages` 找到 `vozeb`，进入 `Package settings -> Change visibility` 设置为 `Public`；私有部署则在服务器执行 `docker login ghcr.io` 后再 `docker compose pull`。

### 2. 首次初始化

打开 `http://服务器IP:3000`，第一次注册的账号会自动成为管理员。这个首个管理员账号用于初始化站点，管理员登录后可进入 `管理员后台`，配置网站标题、Logo、SEO、注册策略、邮箱服务、模型渠道、默认模型、用户额度和公共提示词库。
...
普通用户和管理员都可以从右上角账号菜单进入 `个人资料`，修改昵称、绑定邮箱和登录密码。管理员可在后台 `用户管理` 中修改用户昵称、邮箱、角色、状态、每日额度，必要时重置用户密码或删除用户。系统会阻止删除当前管理员和最后一个管理员，避免后台被锁死。
...
不要执行 `docker compose down -v`，否则会删除 `vozeb-data` 数据卷。升级前也可以在管理员后台概览点击“备份用户数据库”，下载 `.data/auth.json` 和 `.data/prompts.json` 留底。
...
VOZEB 增加了账号系统和后台管理能力。站点首次注册的账号会自动成为管理员，用于完成初始化配置。管理员可以控制注册是否开放、是否要求邮箱注册，调整用户角色、账号状态、每日额度、签到奖励、系统接口渠道和默认模型。后台采用侧边栏切换布局，概览、系统设置、用户管理和公共提示词库分区更清楚。

...
type PromptCategory = {
    category: string;
    githubUrl: string;
    build: () => Promise<Array<Omit<RemotePrompt, "category" | "githubUrl">>>;
};
...
export async function loadOriginalAuthorPrompts() {
    const settled = await Promise.all(
        categories.map(async (category) => {
            try {
                ...
                console.warn(`Load prompt source failed: ${category.githubUrl}`, error);
                return [];
            }
        }),
    );
    const items = settled
        .flat()
        .filter((item) => item.title && item.prompt && isRemoteImageUrl(item.coverUrl))
        .map((item) => ({ ...item, coverUrl: encodeURI(item.coverUrl) }));
    return filterReachableImagePrompts(items);
}
...
async function buildGptImage2Prompts() {
    ...
    const items: Array<Omit<RemotePrompt, "category" | "githubUrl">> = [];
    data.forEach((item) => {
        ...
        items.push(defaultPrompt(`gpt-image-2-prompts-${leftPad(items.length + 1)}`, item.title, prompt, image, tagsFromCategory(item.category || ""), markdownPreview([image])));
    });
    return items;
}
...
async function buildDavidWuGptImage2Prompts() {
    ...
    return data
        ...
        .filter((item): item is Omit<RemotePrompt, "category" | "githubUrl"> => Boolean(item));
}

function defaultPrompt(id: string, title: string, prompt: string, coverUrl: string, tags: string[], preview: string): Omit<RemotePrompt, "category" | "githubUrl"> {
    return { id, title, coverUrl, prompt, tags, preview };
}
...
function extractMarkdownImages(baseUrl: string, markdown: string) {
    ...
    const htmlImages = Array.from(markdown.matchAll(/<img\b[^>]*\bsrc=["']([^"']+)["'][^>]*>/gi), (match) => absoluteImage(baseUrl, match[1]));
    return [...markdownImages, ...htmlImages].filter(isContentImageUrl);
}
...
function isContentImageUrl(value: string) {
    if (!isRemoteImageUrl(value)) return false;
    const normalized = value.toLowerCase();
    if (normalized.includes("img.shields.io") || normalized.includes("awesome.re/badge") || normalized.includes("badge.svg")) return false;
    if (normalized.includes("github.com/") && normalized.includes("/actions/workflows/") && normalized.includes("/badge.svg")) return false;
    if (normalized.includes("star-history.com") || normalized.includes("contrib.rocks")) return false;
    if (normalized.includes("/assets/banner/") || normalized.includes("readme-header") || normalized.includes("prompt-library-screenshot")) return false;
    return true;
}

async function filterReachableImagePrompts(items: RemotePrompt[]) {
    const results: RemotePrompt[] = [];
    const batchSize = 30;
    for (let index = 0; index < items.length; index += batchSize) {
        const batch = items.slice(index, index + batchSize);
        const checks = await Promise.all(batch.map(async (item) => (isTrustedImageHost(item.coverUrl) || (await isReachableImage(item.coverUrl)) ? item : null)));
        results.push(...checks.filter((item): item is RemotePrompt => Boolean(item)));
    }
    return results;
}

function isTrustedImageHost(url: string) {
    try {
        const parsed = new URL(url);
        return ["raw.githubusercontent.com", "github.com", "user-images.githubusercontent.com", "private-user-images.githubusercontent.com", "pbs.twimg.com", "cms-assets.youmind.com", "marketing-assets.youmind.com", "cdn.imgedify.com"].some(
...
async function isReachableImage(url: string) {
    return fetchWithTimeout(url, { method: "HEAD" })
        .then((response) => isImageResponse(response))
        .catch(() =>
            fetchWithTimeout(url, { method: "GET", headers: { Range: "bytes=0-0" } })
                .then((response) => isImageResponse(response))
                .catch(() => false),
        );
}
...
async function fetchWithTimeout(url: string, init: RequestInit) {
    ...
    try {
        return await fetch(url, { ...init, cache: "no-store", redirect: "follow", signal: controller.signal });
    } finally {
        clearTimeout(timer);
    }
}

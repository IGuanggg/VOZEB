...
export async function writeReferenceImageDataUrl(dataUrl: string, options: { persistent?: boolean } = {}): Promise<StoredReferenceAsset> {
    ...
    const token = `${options.persistent ? "canvas-" : ""}${Date.now()}-${randomUUID()}${extensionFromMime(parsed.mimeType)}`;
...
export async function readReferenceAsset(token: string) {
    ...
    try {
        ...
        if (!isPersistentReferenceAssetToken(safeToken) && Date.now() - fileStat.mtimeMs > REFERENCE_ASSET_TTL_MS) {
...
async function cleanupExpiredReferenceAssets() {
    ...
    await Promise.all(
        entries
            .filter((entry) => entry.isFile() && isReferenceAssetToken(entry.name) && !isPersistentReferenceAssetToken(entry.name))
            .map(async (entry) => {
...
function isReferenceAssetToken(value: string) {
    return /^(?:canvas-)?\d{10,}-[0-9a-f-]{36}\.(?:png|jpg|jpeg|webp|gif)$/i.test(value);
}

export function isPersistentReferenceAssetToken(value: string) {
    return /^canvas-\d{10,}-[0-9a-f-]{36}\.(?:png|jpg|jpeg|webp|gif)$/i.test(value);
}

...
import { postDirectorDeskCapturesToHost, subscribeToDirectorDeskCaptureDelivery } from "../io/hostBridge";
...
export function CameraPanel() {
  ...
  const pendingCaptureRequestIdsRef = useRef(new Set<string>());
  ...
  useEffect(() => subscribeToDirectorDeskCaptureDelivery((delivery) => {
    if (!pendingCaptureRequestIdsRef.current.delete(delivery.requestId)) return;
    if (delivery.status === "accepted") {
      setCaptureError(null);
      setCaptureStatus("截图已返回画布");
      return;
    }
    setCaptureStatus(null);
    setCaptureError(delivery.error || "截图返回画布失败");
  }), []);

  const sendCapturesToCanvas = useCallback((items: Array<{ dataUrl: string; fileName?: string }>) => {
    const requestId = postDirectorDeskCapturesToHost(items);
    if (!requestId) return;
    pendingCaptureRequestIdsRef.current.add(requestId);
    setCaptureError(null);
    setCaptureStatus("正在发送到画布...");
  }, []);

  const sendCaptureToCanvas = useCallback((capture: DirectorCameraCapture) => {
    sendCapturesToCanvas([
      {
  ...
  }, [cameraCaptureGroups, sendCapturesToCanvas]);
  ...
  async function handleCameraCapture() {
    try {
      ...
      if (preview) {
        ...
        sendCapturesToCanvas([{ dataUrl: preview.dataUrl, fileName: `${preview.label || currentCamera.name}.png` }]);
      }
    } catch (error) {
      ...
      {activeTab === "properties" ? (
        <>
          ...
          <InspectorSection title="相机截图" className="camera-capture-section">
            ...
            {captureError ? <p>{captureError}</p> : captureStatus ? <p>{captureStatus}</p> : null}
